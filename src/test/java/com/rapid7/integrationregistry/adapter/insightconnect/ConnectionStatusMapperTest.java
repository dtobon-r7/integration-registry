...
class ConnectionStatusMapperTest {
  ...
  void deriveStatus_shouldSelectMostRecentTest_whenMultiplePresent() {
    ...
    List<ConnectionTest> multi =
        List.of(test("success", false, T1), test("failed", false, T3), test("success", false, T2));
  ...
  void deriveStatus_shouldReturnMissingData_whenOrchestratorHealthyAndNoTest() {
    ...
    assertThat(mapper.deriveStatus("healthy", null)).isEqualTo(IntegrationStatus.MISSING_DATA);
    assertThat(mapper.deriveStatus("healthy", List.of())).isEqualTo(IntegrationStatus.MISSING_DATA);
  }
  ...
  void mostRecentByCreatedAt_shouldReturnLatest_whenMultipleTests() {
    ...
    List<ConnectionTest> tests =
        List.of(test("failed", false, T1), test("success", false, T3), test("success", false, T2));
    // Act
  ...
  void deriveLastSuccess_shouldReturnMostRecentSuccessTimestamp_whenSuccessesExist() {
    List<ConnectionTest> tests = List.of(test("success", false, T1), test("success", false, T2));
    assertThat(mapper.deriveLastSuccess(tests)).isEqualTo(T2);
  }

  @Test
  void deriveLastSuccess_shouldReturnNull_whenNoSuccess() {
    List<ConnectionTest> tests = List.of(test("failed", false, T1), test("timeout", false, T2));
  ...
  void deriveLastSuccess_shouldReturnLastSuccess_whenLatestOverallIsFailure() {
    ...
    List<ConnectionTest> tests =
        List.of(test("success", false, T2), test("failed", false, T3), test("failed", false, T1));
    assertThat(mapper.deriveLastSuccess(tests)).isEqualTo(T2);
  }
