 ...
 * mappingVersion()} surface, and {@link #returning(String, VendorResolution)} for tests that need a
...
public final class StubVendorMappingSnapshot {
  ...
  /**
   * Stub whose {@code lookup(...)} always returns {@link VendorResolution#unknown()} — useful for
   * tests that only assert on {@code mappingVersion()} or on side effects of the lookup call.
   */
