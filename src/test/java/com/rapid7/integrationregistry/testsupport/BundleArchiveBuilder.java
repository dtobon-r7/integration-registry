...
/**
 * Builds a single-entry gzipped tarball ({@code .tgz}) for tests that stub the S3 fetch with
 * realistic bytes. Mirrors what T11's bundle-publish pipeline produces: one tar entry named {@code
 * vendor-mapping.yaml} carrying the YAML bytes, gzipped.
 *
 * <p>Tests typically read {@code src/main/resources/vendor-mapping/bundle/mvp-seed.yaml} via {@code
 * getResourceAsStream}, then call {@link #tgzOf(byte[], String)} with {@code "vendor-mapping.yaml"}
 * as the entry name.
 */
