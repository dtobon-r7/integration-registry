...
/**
 * Integration test asserting same-version-restart cache behavior: when a
 * cached {@code .tgz} exists at boot, the loader reads from disk and never
 * touches S3. Pre-seeds the cache file BEFORE Spring's context bootstrap
 * via a static {@code @BeforeAll}, then registers the temp dir into
 * {@code integration-registry.vendor-mapping.cache-dir} via
