...
class BundleLoadListenerTest {
  ...
  void setUp() {
    ...
    properties =
        new VendorMappingProperties("v1.0.0", "test-bucket", "registry/mappings/", Path.of("/tmp"));
  ...
  void onApplicationEvent_shouldLogStructuredError_whenLoadFails() throws Exception {
    // Arrange
    when(loader.load())
        .thenThrow(BundleLoadException.s3FetchFailed(new IOException("connection reset")));
  ...
  @Test
  void onApplicationEvent_shouldAbsorbRuntimeException_whenLoaderThrowsUnchecked()
      throws Exception {
