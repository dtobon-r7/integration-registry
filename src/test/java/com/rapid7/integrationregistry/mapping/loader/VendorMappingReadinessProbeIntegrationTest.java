...
/**
 * HTTP-level readiness probe test. Asserts via {@code RestClient} that the
 * actual {@code /actuator/health/readiness} JSON exposes the {@code bundleLoad}
 * component (the indicator bean's group registration name) once the bundle has
 * loaded successfully.
 *
 * <p>This is the only test in the suite that exercises the probe through the
 * HTTP boundary; the other integration tests assert the indicator bean
 * directly. Hitting HTTP catches a subtle regression class: Spring Boot's
 * {@code HealthContributorNameGenerator} strips the {@code HealthIndicator}
 * suffix when registering bean names into health groups, so the bean
 * {@code bundleLoadHealthIndicator} surfaces in the JSON as {@code bundleLoad}.
 * If a future framework upgrade changed that suffix-stripping rule the
 * application.yaml's {@code group.readiness.include: ...,bundleLoad} would
 * silently match nothing — the readiness gate would fail open and ship traffic
 * to a holder-empty replica. Asserting the JSON shape here pins the contract.
 */
...
class VendorMappingReadinessProbeIntegrationTest {
    ...
    static void seedCacheBeforeContextBootstrap() throws IOException {
        ...
        byte[] mvpSeedYaml;
        try (InputStream stream = VendorMappingReadinessProbeIntegrationTest.class
                .getResourceAsStream("/vendor-mapping/bundle/mvp-seed.yaml")) {
    ...
    void readinessProbe_shouldExposeBundleLoadComponent_overHttp() {
        ...
        assertThat(components)
            .as("readiness group must include bundleLoad component "
