...
class EnumSchemaSyncTest {
  ...
  private static final String CATEGORY_ENUM_POINTER =
      "/$defs/VendorService/properties/category/enum";
  private static final String SOURCE_TYPE_ENUM_POINTER =
      "/$defs/DataSource/properties/source_type/enum";
  ...
  void schemaCategoryEnum_shouldMatchVendorCategoryWireForms_whenInspected() {
    // Arrange
    Set<String> javaWireForms =
        Stream.of(VendorCategory.values())
            .map(VendorCategory::wireForm)
            .collect(Collectors.toSet());

    // Act
    Set<String> schemaWireForms =
        BundleSchemaResources.textValuesAt(schemaRoot, CATEGORY_ENUM_POINTER);
  ...
  void schemaSourceTypeEnum_shouldMatchSourceTypeWireForms_whenInspected() {
    // Arrange
    Set<String> javaWireForms =
        Stream.of(SourceType.values()).map(SourceType::wireForm).collect(Collectors.toSet());

    // Act
    Set<String> schemaWireForms =
        BundleSchemaResources.textValuesAt(schemaRoot, SOURCE_TYPE_ENUM_POINTER);
  ...
  void schemaProductEnum_shouldMatchProductNameWireForms_whenInspected() {
    // Arrange
    Set<String> javaWireForms =
        Stream.of(ProductName.values()).map(ProductName::wireForm).collect(Collectors.toSet());

    // Act
    Set<String> schemaWireForms =
        BundleSchemaResources.textValuesAt(schemaRoot, PRODUCT_ENUM_POINTER);
