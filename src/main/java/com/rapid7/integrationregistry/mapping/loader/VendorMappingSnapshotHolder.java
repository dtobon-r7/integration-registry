...
/**
 * One-shot {@link AtomicReference}-backed wrapper exposed as the {@link VendorMappingSnapshot}
 * bean. The runtime loader populates the reference exactly once during boot via {@link
 * #set(VendorMappingSnapshot)}; pre-population reads throw {@link IllegalStateException}.
 *
 * <p>The readiness gate is the upstream contract that prevents pre-load reads in production. The
 * {@code IllegalStateException} is a defensive failure mode for the case where the gate is bypassed
 * (e.g., a misconfigured probe).
 */
final class VendorMappingSnapshotHolder implements VendorMappingSnapshot {
  ...
  /**
   * Non-throwing check used by the readiness {@code HealthIndicator} to report DOWN before the
   * bundle has been loaded. Distinct from {@link #lookup} / {@link #mappingVersion}, which throw to
   * signal a misconfigured probe.
   */
  boolean isLoaded() {
    return ref.get() != null;
  }
