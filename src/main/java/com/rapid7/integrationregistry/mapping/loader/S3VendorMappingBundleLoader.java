...
/**
 * Cache-first runtime loader that produces a {@link VendorMappingSnapshot} at boot. The cache is a
 * same-version-restart optimization (per RFC-001 §Bundle lifecycle): cache hit reads the local
 * copy; cache corruption (bad gzip / wrong tar entry / parse failure) deletes the file and falls
 * through to S3; cache-read I/O errors propagate as {@link BundleLoadException}.
 *
 * <p>Cache writes are atomic: bytes are written to a sibling temp file and then renamed via {@link
 * Files#move(Path, Path, java.nio.file.CopyOption...)} with {@code ATOMIC_MOVE}, so a crash
 * mid-write leaves either the previous cache or no cache, never a partial file.
 *
 * <p>The bundle artifact is a {@code .tgz} (gzipped tarball) with a single entry named {@code
 * vendor-mapping.yaml}; the YAML is then handed to the parser. Multi-entry tarballs and
 * non-standard entry names are rejected.
 *
 * <p>Two byte-budget caps defend against an outsized payload OOM-ing every replica at boot: a 50 MB
 * compressed cap on the S3 fetch and a 200 MB inflated cap on the gzip stream (gzip-bomb defence).
 * Both are hard rejects surfaced as {@code archiveExtractFailed}.
 */
final class S3VendorMappingBundleLoader {
  ...
  /**
   * Hard cap on the compressed bundle byte count fetched from S3. The current MVP-seed tarball is
   * well under 10 KB; 50 MB is generous head-room for future fixture growth while still bounding
   * boot-time memory if S3 returns an unexpectedly large payload.
   */
  private static final long MAX_BUNDLE_BYTES_COMPRESSED = 50L * 1024 * 1024;

  /**
   * Hard cap on the inflated tar bytes streamed through the gzip decoder. Gzip-bomb defence: {@code
   * BoundedInputStream} returns EOF at the cap rather than throwing, so an oversized payload
   * surfaces as an {@code IOException} from the tar parser and is wrapped in {@code
  ...
  VendorMappingSnapshot load() throws BundleLoadException {
    ...
    try {
      ...
      try {
        ...
        log.warn(
            "vendor mapping disk cache corrupted at {}, deleting and falling through to S3 — {}",
            cacheFile,
            corruptCache.getMessage());
      ...
      if (bytes.length > MAX_BUNDLE_BYTES_COMPRESSED) {
        throw BundleLoadException.archiveExtractFailed(
            new IllegalStateException(
                "Bundle exceeds compressed size cap: "
                    + bytes.length
                    + " > "
                    + MAX_BUNDLE_BYTES_COMPRESSED
                    + " bytes"));
      }
      return bytes;
  ...
  private void writeCacheAtomically(Path cacheFile, byte[] bytes) throws BundleLoadException {
    try {
      ...
      Files.move(
          temp, cacheFile, StandardCopyOption.ATOMIC_MOVE, StandardCopyOption.REPLACE_EXISTING);
  ...
  private VendorMappingSnapshot parseFromBytes(byte[] tgzBytes) throws BundleLoadException {
    ...
    try (ByteArrayInputStream tarByteIn = new ByteArrayInputStream(tarBytes);
      ...
      if (tarIn.getNextEntry() != null) {
        throw BundleLoadException.archiveExtractFailed(
            new IllegalStateException(
                "tarball has more than one entry; expected exactly " + BUNDLE_ENTRY_NAME));
      }
      return snapshot;
    } catch (IOException ex) {
  ...
  private VendorMappingSnapshot parseSnapshot(TarArchiveInputStream tarIn)
      throws BundleLoadException {
    try {
  ...
  private static void requireSingleNamedEntry(TarArchiveInputStream tarIn)
      throws BundleLoadException, IOException {
    TarArchiveEntry entry = tarIn.getNextEntry();
    if (entry == null) {
      throw BundleLoadException.archiveExtractFailed(new IllegalStateException("tarball is empty"));
    }
    if (!BUNDLE_ENTRY_NAME.equals(entry.getName())) {
      throw BundleLoadException.archiveExtractFailed(
          new IllegalStateException(
              "expected entry " + BUNDLE_ENTRY_NAME + " but found " + entry.getName()));
    }
  }
  ...
  private void deleteCorruptCache(Path cacheFile) {
    try {
      ...
      log.warn(
          "failed to delete corrupt cache file at {}; will overwrite via atomic move",
          cacheFile,
          ioe);
    }
  }
