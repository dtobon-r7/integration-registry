 ...
 * Stateless and framework-agnostic; the runtime loader (Spring-wired in a downstream layer)
 * supplies the input stream and handles S3 fetch, disk caching, and the readiness gate.
 *
 * <h2>Threading</h2>
 *
 ...
 *
 * <p>The {@link #parse(java.io.InputStream)} entry point reads the entire stream into memory via
 * Jackson's YAML factory. The bundle format (defined by RFC-001 §Vendor mapping) is small by design
 * — vendor / vendor-service / data-source counts are bounded by the curated catalog, not by user
 * input — so the parser does not impose explicit size, depth, or alias caps. The trust boundary is
 * owned by the runtime loader's S3 fetch (the bundle is curated in-repo and shipped via a CI
 * publish pipeline; the loader is responsible for any threat-model hardening such as SnakeYAML
...
public final class BundleParser {
   ...
   *
   * <ul>
   *   <li><b>Bundle-data failures</b> — surface as {@link BundleParseException} (checked).
   *       YAML-syntax errors carry the underlying Jackson exception as their cause;
   *       schema-validation failures carry the validator's structured {@code
   *       Set<ValidationMessage>} via {@link BundleParseException#validationMessages()}.
   *   <li><b>Schema/enum-sync drift</b> — propagates as {@link IllegalStateException} (unchecked)
   *       when a schema-validated wire form does not map through {@code fromWireForm()}. This is a
   *       build-time invariant violation that {@code EnumSchemaSyncTest} is the first line of
   *       defense against; it is NOT caught by {@code BundleParseException}. Callers that need to
   *       handle every parse failure (e.g. the readiness-gate loader) must catch it explicitly.
   *   <li><b>Packaging defects</b> — propagate as {@link IllegalStateException} (unchecked) when
   *       the bundled JSON Schema resource is missing or unreadable from the classpath. Same
   *       caveat: not caught by {@code BundleParseException}.
   * </ul>
   *
   * <p>An empty {@code services} array on a vendor (or an empty {@code data_sources} array on a
   * service) contributes zero index entries to the resulting snapshot. The schema makes both arrays
   * {@code required}; the parser tolerates them as legal empty placeholders for vendors / services
   * staged across multiple bundle revisions.
   *
   * @throws BundleParseException if the YAML is unparseable or the parsed document fails JSON
   *     Schema validation.
   * @throws IllegalStateException if a schema/enum invariant is violated or the bundled schema
   *     resource is missing/unreadable. See the failure classes above.
  ...
  private static <E extends Enum<E>> E requireEnum(
      Optional<E> resolved, String fieldName, String wireForm) {
    return resolved.orElseThrow(
        () ->
            new IllegalStateException(
                "Schema-validated bundle contained "
                    + fieldName
                    + "="
                    + wireForm
    ...
    try (InputStream in = BundleParser.class.getResourceAsStream(SCHEMA_CLASSPATH)) {
      if (in == null) {
        throw new IllegalStateException(
            "Bundle schema resource missing from classpath: "
                + SCHEMA_CLASSPATH
