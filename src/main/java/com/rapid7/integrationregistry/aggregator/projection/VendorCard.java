...
/**
 * Lightweight feed for the {@code GET /vendors} filter dropdown — vendor identity plus the count of
 * vendor services with at least one integration in the requesting org. Per RFC-001 §Read API
 * Contract → Projections this projection is intentionally narrow: no {@code aggregate_health}, no
 * {@code vendor_category}.
 */
public record VendorCard(String vendorId, String vendorName, int vendorServicesCount) {
