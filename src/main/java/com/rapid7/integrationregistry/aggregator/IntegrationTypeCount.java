...
/**
 * Per-type aggregation under a vendor service per RFC-001 §Integration Types — one entry per
 * distinct {@code integration_type}. The surface is intentionally narrow: only {@code total} and
 * {@code errorCount}; no {@code warning_count} or other state breakdowns. Adding more counts is a
 * non-breaking forward extension.
 */
public record IntegrationTypeCount(String integrationType, int total, int errorCount) {
