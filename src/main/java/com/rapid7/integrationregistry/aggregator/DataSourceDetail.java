...
/**
 * Per-data-source row for {@code GET /vendor-services/{id}} per RFC-001 §Data Source entity.
 * Carries a nested {@code integrations[]} of per-instance detail. The {@code integrationsCount ==
 * integrations.size()} invariant is enforced in the compact constructor — the field exists on the
 * wire because the RFC commits to it, but the two values cannot diverge.
 */
