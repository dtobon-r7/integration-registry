...
/**
 * Thrown when the upstream returns a 4xx authentication/authorization failure
 * (typically 401/403). Mapped by the fan-out coordinator to
 * {@code unavailable_products[].reason = "auth_failure"} (RFC-001).
 *
 * <p>This is a permanent failure — {@link #isTransient()} returns {@code false}
 * — so the coordinator does NOT serve stale-tier data on this exception.
 * See ADR-001 for the family-parent rule.
 */
public class AdapterAuthException extends AdapterException {
     ...
     * {@code WebClientResponseException} for a 401 response from
     * {@code WebClient}); use {@link #AdapterAuthException(String)} only for
