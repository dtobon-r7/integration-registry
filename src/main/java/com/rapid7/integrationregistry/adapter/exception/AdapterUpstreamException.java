...
/**
 * Thrown when the upstream returns a 5xx error or the adapter classifies a
 * transport error as 'upstream broken'. Mapped by the fan-out coordinator to
...
public class AdapterUpstreamException extends AdapterException {
     ...
     * {@code WebClientResponseException} for a 503); use
     * {@link #AdapterUpstreamException(String)} only for adapter-internal
     * failures with no underlying exception. Preserving the cause chain is
     * required for downstream debugging.
     */
