...
/**
 * Thrown when the adapter's own timeout fires (the upstream did not respond in time). Mapped by the
 * fan-out coordinator to {@code unavailable_products[].reason = "timeout"} (RFC-001).
 *
...
public class AdapterTimeoutException extends AdapterException {
   ...
   * SocketTimeoutException}, {@code WebClientResponseException} for a 408); use {@link
   * #AdapterTimeoutException(String)} only for adapter-internal failures with no underlying
   * exception. Preserving the cause chain is required for downstream debugging.
   */
