...
/**
 * Pure health-derivation logic for ICON connections (RFC-001 §InsightConnectAdapter). No Spring, no
 * HTTP, no clock — unit-testable in isolation. Owns the five-state status table, the instance-level
 * precedence ({@code error > missing_data > warning > disabled > healthy}), most-recent {@link
 * ConnectionTest} selection, and {@code last_success_timestamp} derivation.
 */
public class ConnectionStatusMapper {
  ...
  /**
   * Derive the instance status from the orchestrator status and the connection's tests. The most
   * recent test by {@code createdAt} drives the test-derived signals; {@code tests} may be
   * null/empty when no tests exist. First match wins, in precedence order. An unrecognized or
   * {@code null} orchestrator value falls back to {@link IntegrationStatus#MISSING_DATA} with a
   * WARN log — one anomalous connection must not fail the fetch.
   *
   * <p>Selecting the most-recent test is done here rather than by the caller, so there is no
   * two-step protocol a caller could get wrong by passing a non-most-recent test.
   */
  ...
  /**
   * Resolve the warning / disabled / healthy / missing_data-tail rules, which depend on the
   * orchestrator status (plus a confirming test for healthy). {@code orch} / {@code testStatus} are
   * normalized; {@code rawOrch} is the original value, logged verbatim when unrecognized.
   */
  ...
  /**
   * The most recent test by {@code createdAt}, or {@code null} when the list is null/empty. Tests
   * with a null {@code createdAt} sort earliest. Package-private: used internally by {@link
   * #deriveStatus} and exercised directly by the unit tests; not part of the public mapper surface.
   */
    ...
    return tests.stream()
        .max(
            Comparator.comparing(
                ConnectionTest::createdAt, Comparator.nullsFirst(Comparator.naturalOrder())))
        .orElse(null);
  }

  /**
   * Timestamp of the most recent successful test, or {@code null} when none succeeded (or the list
   * is null/empty). Independent of {@link #mostRecentByCreatedAt} — a connection whose latest test
   * failed still reports its last success.
   */
